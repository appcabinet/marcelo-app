    ...
    return (
        <>
            ...
            <ul className={"w-full ml-0.5"}>
                {posts.map(p => (
                    <li key={p.id}
                        className={"w-full text-neutral-500 mb-1 md:mb-1 flex justify-start content-center items-center"}>
                        ...
                        <Link href={`/w/${p.slug}`}
                              className={"hover:underline hover:text-orange-500 w-full text-md md:text-lg"}>
                            {p.title}
                        </Link>
                    </li>
                ))}
